...
# \todo\think Make this standard class to be instantiated?
class app:
    ...
    class jog(State):
        ...
        def next():
            ...
            t = time.time()
            if(not app.grbl.jog(s*app.x, s*app.y, s*app.z, app.v)):
                app.input.rumble()
            app.dt -= time.time() - t
    ...
    def loop():
        try:
            while True:
                app.x, app.y, app.z, app.v = app.grbl.xyzv_from_ax(*app.input.xyz(), app.speed.value())
                app.fsm.event()
                app.speed.fsm.event()
                if app.dt > 0.0:
                    time.sleep(app.dt)
